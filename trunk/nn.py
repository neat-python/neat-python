...
class Neuron:
    ...
    def __init__(self, neuron_type, response=1.0):
        self.synapses = []
        self.neuron_type = neuron_type # input, hidden, output, bias
        self.response = response
        if self.neuron_type is 'bias':
            self.activation = -1.0
        else:
...
class Network:
	...
	def aexecute(self, inputs):	   
		...
		if(n.neuron_type == 'output'): output.append(n.activation)
...
if __name__ == "__main__":
    # Example
    nn = FeedForward(2,1,3,True)
    print 'Parallel activation method: '
    for t in range(3):
	print nn.sexecute([1,0])
		
    print 'Serial activation method: '
    for t in range(3):
	    print nn.aexecute([1,0])
